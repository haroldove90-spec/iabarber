...
const StepCard: React.FC<{ icon: React.ReactNode; title: string; description: string; stepNumber: number }> = ({ icon, title, description, stepNumber }) => {
    return (
        <div className="relative text-center p-6 bg-gray-900 rounded-lg shadow-lg">
            <div className="absolute top-3 right-3 w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center text-black font-bold text-lg ring-4 ring-gray-900">
                {stepNumber}
            </div>
            <div className="w-20 h-20 mx-auto mb-4 bg-gray-800 rounded-full flex items-center justify-center text-gray-300">
                {icon}
            </div>
